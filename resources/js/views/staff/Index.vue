        ...
        <template v-slot:top>
          ...
          >
            ...
            <v-dialog v-model="dialog" max-width="500px">
              <v-card>
                <v-card-title class="headline">Are you sure you want to delete this Staffs?</v-card-title>
                <v-card-actions>
                  <v-spacer></v-spacer>
...
import { mapActions, mapGetters } from 'vuex'
  export default {
    ...
    methods: {
      ...
      editItem (item) {
        this.$router.push(`/staff/${item.id}/edit`)
      },
      closeDelete () {
        this.dialog = false
      ...
      searchStaffs(){
        if(this.search === ''){
          ...
          this.staffsData = this.staffsData.filter( staffs => staffs.name.toUpperCase().includes(this.search.toUpperCase()))
        }
      }
    },
    created(){
